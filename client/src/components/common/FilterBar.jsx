    ...
    <div className={`space-y-3 ${className}`}>
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:text-white dark:placeholder-gray-500 transition-all duration-200"
          />
          {searchValue && (
            <button
                    ...
                    {filterOptions.map((option) => (
                      ...
                      >
                        <span className="flex items-center justify-between">
                          {option.label}
                          {filterValue === option.value && <Check className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />}
                        </span>
                      </button>
                    ))}
                  </div>
      ...
      {!isMobile && activeFilterCount > 0 && !showFilters && (
        <div className="flex flex-wrap gap-2">
          {searchValue && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full">
              Search: "{searchValue.slice(0, 20)}"
              <button onClick={() => onSearchChange("")} className="hover:text-indigo-900">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {filterValue && filterValue !== "all" && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full">
              {selectedFilterLabel}
              <button onClick={() => onFilterChange("all")} className="hover:text-indigo-900">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
